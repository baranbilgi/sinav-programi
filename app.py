...
def flexible_time_parse(time_str):
    """Excel'deki hatalı saat formatlarını (14:15:16:00 vb.) temizler ve ayırır."""
    if not time_str or pd.isna(time_str): return None, None
    s = str(time_str).replace('.', ':').strip()
    # Sayı ve iki nokta dışındaki her şeyi temizle
    clean = re.sub(r'[^0-9:]', '', s)
    
    # 14:15:16:00 durumunu 14:15 ve 16:00 olarak ayır
    parts = clean.split(':')
    if len(parts) >= 4:
    ...
    if st.sidebar.button("Optimizasyon Sürecini Başlat"):
        if sum(w.values()) != 100: st.sidebar.error("⚠️ Toplam 100 olmalı.")
            ...
            with st.spinner('Planlama oluşturuluyor...'):
                ...
                # Puanlama (Kısıtlı personeller belirli seans dengelerinden muaf)
                ...
                if solver.Solve(model) in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
                    ...
                    for i in invs:
                        st.session_state.stats.append({
                            ...
                            "Top. Süre": solver.Value(total_mins[i]), "Toplam Görev": solver.Value(total_exams[i]),
                            "Sabah Seansı": solver.Value(morn_cnt[i]), "Akşam Seansı": solver.Value(eve_cnt[i])
                        })
                    st.success("✅ İşlem tamamlandı.")
                else: st.error("❌ Uygun plan bulunamadı. Lütfen muafiyetleri azaltın veya personel sayısını artırın.")

if st.session_state.results:
    res_df = pd.DataFrame(st.session_state.results)
    tab1, tab2, tab3 = st.tabs(["📋 Çizelge", "📊 Dağılım Analizi", "📖 Metodoloji"])
    with tab1:
